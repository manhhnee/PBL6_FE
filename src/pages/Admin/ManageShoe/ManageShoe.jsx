...
function ManageShoe() {
  ...
  const getshoes = async () => {
    try {
      const response = await axios.get('http://54.164.6.175:4000/api/shoes?limit=10000');
  ...
  const fetchApiBrand = async () => {
    const response = await axios.get('http://54.164.6.175:4000/api/brand');
  ...
  const handleAddshoe = async (idCategory, idBrand, name, price, import_price, description, color, image) => {
    await axios
      .post(
        'http://54.164.6.175:4000/api/shoes/add',
        {
  ...
  const handleAddShoeSize = async (size, amount) => {
    try {
      await axios
        .post(
          `http://54.164.6.175:4000/api/shoes/add_size/${shoeIDSize}`,
          {
            amount: amount,
            size: size,
          },
  ...
  const handleUpdateshoe = async (id, idCategory, idBrand, name, size, color, amount, description) => {
    if (!validateForm()) {
      ...
      await axios
        .put(
          `http://54.164.6.175:4000/api/shoes/updateInfor/${id}`,
          {
            id_category: idCategory,
            id_brand: idBrand,
            name: name,
  ...
  const handleDeleteshoe = async (id) => {
    await axios
      .delete(`http://54.164.6.175:4000/api/shoes/delete/${id}`, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${GetToken()}`,
        },
