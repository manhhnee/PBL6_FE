...
function ManageCategory() {
  ...
  const handleAddCategory = async (name, image) => {
    await axios
      .post(
        'https://2hm-store.click/api/category/add',
        {
          name: name,
          image: image,
        },
  ...
  const handleDeleteCategory = async () => {
    await axios
      .delete(`https://2hm-store.click/api/category/delele/${selectedCategoryId}`, {
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchApiShoes = async () => {
      const response = await axios.get(`https://2hm-store.click/api/shoes?category=${selectedCategoryId}`, {
        headers: {
          Authorization: `Bearer ${GetToken()}`,
        },
      });
